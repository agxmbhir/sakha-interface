...
import { ConnectionParams, ConnectorListItemDTO, SingleAppInfoResDTO } from 'composio-core';
...
export async function GET() {
    try {
        ...
        let integrations = await toolset.integrations.list();

        if (!integrations) {
            throw new Error('Failed to fetch integrations from Composio.');
        }

        // Given the list of integrations, filter out the integrations that are already connected. 
        // now use the availableIntegrations to fetch the app details    
